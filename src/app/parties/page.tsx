...
export default function PartiesPage() {
  ...
  const refresh = useCallback(() => {
    if (!companyId) return;
    api<{ data: PartyRow[] }>(`/api/parties?companyId=${companyId}`)
      .then(({ data }) => setParties(data))
      .catch((e: ApiError) => toast(e.detail, "error"))
  ...
  async function handleDelete() {
    ...
    try {
      await api(`/api/parties/${deleteTarget.id}`, { method: "DELETE" });
      toast("Party deleted.");
      refresh();
    } catch (err) {
        ...
        <DataTable
          ...
          columns={[
            ...
            },
            {
              header: "VAT",
              cell: (p) => (
                <span className="tabular-amount">{p.vatNumber ?? "–"}</span>
              ),
            },
            {
              header: "Location",
              cell: (p) => (
                <span className="text-muted">{p.locationName ?? "–"}</span>
              ),
            },
            {
              header: "Phone",
              cell: (p) => (
                <span className="tabular-amount">{p.phone ?? "–"}</span>
              ),
            },
            {
              header: "Created",
          ...
          mobileCard={(p) => (
            <>
              ...
              {p.vatNumber && (
                <p className="text-sm text-muted">VAT: {p.vatNumber}</p>
              )}
              {p.phone && (
                <p className="text-sm text-muted">Phone: {p.phone}</p>
              )}
              {p.locationName && (
