...
export function StatusBadge({ status }: { status: LedgerRowStatus }) {
  ...
  if (status === STATUS_DUPLICATE) {
    return (
      <span className="inline-flex items-center gap-1 rounded-full bg-warning/10 px-2 py-0.5 text-xs font-medium text-warning" title="Duplicate invoice for this party">
        <svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor">
